...
from pleco_target.pleco_target_pb2_grpc import K8sGWStub
...
def handle_leap_to_new_cluster(sources_doc, step_doc):
    ...
    p = os.popen("kubectl --context %s -n %s get pods -o custom-columns=PodName:.metadata.name | grep %s" % (leader_source['context'],ns, resource_name))
    ...
    with open(r"script0_leader.js", 'w') as file:
        documents = yaml.dump(
            "var cfg = rs.conf(); cfg.members[0].host='%s:27017'; rs.reconfig(cfg, {force : true} )"%mongo_leader_host, file)
    os.system("kubectl --context %s cp -n %s script0_leader.js /%s:." % (leader_source['context'], ns, mongo_pod_leader))
    os.system("kubectl --context %s exec -it %s -n %s -- mongo script0_leader.js" % (leader_source['context'],mongo_pod_leader,ns))
    ...
    os.system("kubectl --context %s cp -n %s script1_leader.js /%s:." % (leader_source['context'], ns, mongo_pod_leader))
    os.system("kubectl --context %s exec -it %s -n %s -- mongo script1_leader.js" % (leader_source['context'],mongo_pod_leader,ns))
    ...
    os.system("sleep 10s")
    os.system("kubectl --context %s cp -n %s script2_leader.js /%s:." % (leader_source['context'], ns, mongo_pod_leader))
    os.system("kubectl --context %s exec -it %s -n %s -- mongo script2_leader.js" % (leader_source['context'],mongo_pod_leader,ns))
    ...
    os.system("sleep 10s")
    os.system("kubectl --context %s cp -n %s script1_follower.js /%s:." % (follower_source['context'], ns, mongo_pod_follower))
    os.system("kubectl --context %s exec -it %s -n %s -- mongo script1_follower.js" % (follower_source['context'],mongo_pod_follower,ns))
    ...
    os.system("kubectl --context %s cp -n %s script2_follower.js /%s:." % (leader_source['context'], ns, mongo_pod_leader))
    os.system("kubectl --context %s exec -it %s -n %s -- mongo script2_follower.js" % (leader_source['context'],mongo_pod_leader,ns))
