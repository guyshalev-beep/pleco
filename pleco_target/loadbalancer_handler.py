...
def handle_leap(sources_doc, step_doc):
    ...
    endpoind_group_name_follower = "%s%s" % (follower_source['endpoint_group_name'], follower_source['suffix'])
    endpoind_group_name_leader = "%s%s" % (leader_source['endpoint_group_name'], leader_source['suffix'])
    backend_service = step_doc['resource']['backend_service']
    target_ip = step_doc['resource']['target_ip']
    ...
    os.system(
        "gcloud compute network-endpoint-groups create %s --global --network-endpoint-type=\"internet-ip-port\" --default-port=%s" % endpoind_group_name_follower,target_ip)
    # Set up the IP of the new Cluster in the newly created endpoint group
    os.system("gcloud compute network-endpoint-groups update %s --global --add-endpoint=ip=%s,port=%s" % (
    endpoind_group_name_follower, target_ip))
...
class LoadBalancerHandler(object):
    def __init__(self):
        print("start LoadBalancerHandler")
        pass
