...
class PlecoService(object):
    ...
    def create_follower(cluster_leader: DataClassCluster, cluster_follower: DataClassCluster, project_id, user_name):
        ...
        os.system("terraform -chdir=%s/terraform/terraform_co_cluster/cluster_install init"%home_path)
        print ("finished init")
        os.system(
            "terraform -chdir=%s/terraform/terraform_co_cluster/cluster_install apply -var='suffix_name=%s' -var='zone=%s' -var='region=%s' -var='project_id=%s' -auto-approve" % (
            home_path,cluster_follower.suffix, cluster_follower.zone, cluster_follower.region, project_id))
        ...
        os.system("terraform -chdir=%s/terraform/terraform_co_cluster/firewall_allow init"%home_path)
        print ("stop init")
        os.system(
            "terraform -chdir=%s/terraform/terraform_co_cluster/firewall_allow apply -var='suffix_name=%s' -var='zone=%s' -var='region=%s' -var='project_id=%s' -auto-approve" %(
            home_path,cluster_follower.suffix, cluster_follower.zone, cluster_follower.region, project_id))
    ...
    def install_istio(cluster_leader: DataClassCluster, cluster_follower: DataClassCluster, project_id, user_name):
        ...
        # rolebinding - NOT SURE IF NEEDED HERE
        ...
        s="kubectl --context %s create secret generic cacerts -n istio-system --from-file=%s/istio-1.10.0/samples/certs/ca-cert.pem --from-file=%s/istio-1.10.0/samples/certs/ca-key.pem --from-file=%s/istio-1.10.0/samples/certs/root-cert.pem --from-file=%s/istio-1.10.0/samples/certs/cert-chain.pem;"%(cluster_follower.context, home_path,home_path,home_path,home_path)
        os.system(s)
        # Set Default Network
        ...
        with open(r"istio_install.yaml") as file:
            ...
            os.system("sleep 10s")
        # Expose Services
        s = "kubectl --context=%s apply -n istio-system -f %s/istio-1.10.0/samples/multicluster/expose-services.yaml"%(cluster_follower.context,home_path)
        print (s)
        os.system(s)
    ...
    def install_pleco(cluster: DataClassCluster, project_id, user_name):
        def printMsg(*msgs):
            ...
            for msg in msgs:
                os.system("echo ---- %s " % msg)
            os.system("echo ---------------------------------------------")
        print ("start install pleco")
        s="kubectl get nodes --context %s -o json | jq -r '.items[0]  | [.spec.nodeName,.metadata.name]' | grep gke | sed 's/\"//g'" % cluster.context
        print(s)
        # kubectl get nodes --context gke_acme-331809_europe-west2-b_acme-331809-moon -o json | jq -r '.items[0]  | [.spec.nodeName,.metadata.name]' | grep gke | sed 's"/g'
        p = os.popen(s)
        nodename = p.read()[2:-1]
        print ("nodename %s"%nodename)
        s = "kubectl --context %s get node %s -o jsonpath='{.status.addresses[?(@.type==\"%s\")].address}'" % (
        cluster.context, nodename, "InternalIP")
        p = os.popen(s)
        print (s)
        cluster.internalIP = p.read()
        print ("cluster.internalIP %s"%cluster.internalIP)
        command1 = "docker run -d -p %s:50051:50051 guypleco/gcp:latest" % cluster.internalIP
        s = "gcloud beta compute ssh --zone \"%s\" \"%s\"  --project \"%s\" --command=\"%s\"" % (
